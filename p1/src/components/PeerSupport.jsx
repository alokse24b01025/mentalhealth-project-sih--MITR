...
const PostItem = ({ post, onUpdate }) => {
  ...
  const handleUpvote = async () => {
    try {
      const res = await fetch(`https://mentalhealth-project-sih-mitr.onrender.commentalhealth-project-sih-mitr.onrender.com/api/posts/${post._id}/upvote`, { method: 'PUT' });
      if (res.ok) onUpdate();
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`https://mentalhealth-project-sih-mitr.onrender.commentalhealth-project-sih-mitr.onrender.com/api/posts/${post._id}/comment`, {
        method: 'POST',
        ...
        {post.image && (
          <div className="mb-4 rounded-xl overflow-hidden border border-slate-800 bg-black shadow-inner">
            {post.image.match(/\.(mp4|webm|ogg|mov)$/i) ? (
              <video 
                src={`https://mentalhealth-project-sih-mitr.onrender.commentalhealth-project-sih-mitr.onrender.com${post.image}`} 
                controls 
...
const PeerSupport = () => {
  ...
  const fetchPosts = async () => {
    try {
      const res = await fetch('https://mentalhealth-project-sih-mitr.onrender.commentalhealth-project-sih-mitr.onrender.com/api/posts');
  ...
  const handlePost = async (e) => {
    ...
    try {
      const res = await fetch('https://mentalhealth-project-sih-mitr.onrender.commentalhealth-project-sih-mitr.onrender.com/api/posts/create', {
        method: 'POST',
